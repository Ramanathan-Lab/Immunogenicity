...
export default function ImmunogenicityDetailsPage() {
  ...
  console.log("Received productid in ImmunogenicityDetailsPage:", productid); 
  ...
  const fetchRecordDetails = async () => {
    try {
      console.log("Fetching immunogenicity details for:", { productid });
      ...
      setRecord(response.data.immunogenicity || {});
    } catch (err) {
      console.error("Error fetching details:", err.response?.data || err.message);
      setError("Failed to load immunogenicity details.");
    }
    setLoading(false);
  };
        ...
        <Card sx={{ width: '60%', padding: 3 }}>
          <CardContent>
            ...
            {loading ? (
              ...
              <Typography sx={{ textAlign: 'center' }}>No immunogenicity details available.</Typography>
            )}
